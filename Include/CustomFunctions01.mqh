...
double GetStopLossPrice(bool bIsLongPosition, double entryPrice, int maxLossInPips)
  {
   ...
   if(bIsLongPosition)
     {
      stopLossPrice = entryPrice - maxLossInPips * GetPipValue(); // changed from hardcoded 0.0001
     }
   else
     {
      stopLossPrice = entryPrice + maxLossInPips * GetPipValue(); // changed from hardcoded 0.0001
...
//|                                                                  |
//+------------------------------------------------------------------+
double _OptimalLotSize(double maxRiskPrc, int maxLossInPips)
  {

   double accEquity = AccountEquity();
   Print("accEquityy: " + accEquity);
   
   if (accEquity <= 0) {
      Alert("Your Account Equity is 0!");
      return 0;
   }
...
double OptimalLotSize(double maxRiskPrc, double entryPrice, double stopLoss)
  {
   ...
   return _OptimalLotSize(maxRiskPrc,maxLossInPips);
  }
